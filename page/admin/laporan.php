...
<?php
$q = mysqli_query($con, "SELECT * FROM chekout WHERE status_terima='Terkonfirmasi'");
@$act = $_GET['act'];
...
if ($act == "detail") {
    ...
    while ($c = mysqli_fetch_array($q)) {
        echo '<tr>
            <td style="color: #ffffff"><center>' . $c['nama'] . '</center></td>
            <td style="color: #ffffff"><center>' . $c['tanggal'] . '</center></td>
            <td style="color: #ffffff"><center>' . $c['status_terima'] . '</center></td>
            <td style="color: #ffffff"><center><a href="index.php?page=laporan&act=detail&id=' . $c['id_pembeli'] . '"><span class="glyphicon glyphicon-eye-open"></span></a> | <a href="konfirmasi_transaksi.php?id=' . $c['id_chekout'] . '&id_pembeli=' . $c['id_pembeli'] . '"><span class="glyphicon glyphicon-check"></span></a></center></td>
        </tr>';
    }
